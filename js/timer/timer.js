...
// Timed Stasis / Law of Large Numbers next-fire timestamps and the
// low-health vignette poll handle. Module-owned state (was loose
// window._* globals read/written only here).
let _timedStasisNext = null;    // Timestamp for the next Timed Stasis proc
let _lawOfLargeNext = null;     // Timestamp for the next Law of Large Numbers proc
let _healthVignettePoll = null; // Low-health vignette polling interval id

// Live accessors preserve shared timer state and passive-tree patch seams.
try { Object.defineProperty(globalThis, '_lowTimeLastSecs', { get() { return _lowTimeLastSecs; }, set(v) { _lowTimeLastSecs = v; }, configurable: true }); } catch (e) {}
try { Object.defineProperty(globalThis, 'timerSecs', { get() { return timerSecs; }, set(v) { timerSecs = v; }, configurable: true }); } catch (e) {}
try { Object.defineProperty(globalThis, 'startTimer', { get() { return startTimer; }, set(v) { startTimer = v; }, configurable: true }); } catch (e) {} // PHASE3-SHIM write-through: passive-tree-expansion patch()
try { Object.defineProperty(globalThis, 'stopTimer', { get() { return stopTimer; }, set(v) { stopTimer = v; }, configurable: true }); } catch (e) {} // PHASE3-SHIM write-through: passive-tree-expansion patch()
try { Object.defineProperty(globalThis, 'pauseTimer', { get() { return pauseTimer; }, set(v) { pauseTimer = v; }, configurable: true }); } catch (e) {} // PHASE3-SHIM write-through: passive-tree-expansion patch()
try { Object.defineProperty(globalThis, '_triggerLawOfLargeNumbers', { get() { return _triggerLawOfLargeNumbers; }, set(v) { _triggerLawOfLargeNumbers = v; }, configurable: true }); } catch (e) {} // PHASE3-SHIM write-through: passive-tree-expansion patch()
//------------------------------------------------------------------------
//-------------------CONSTANTS & STATE------------------------------------
//------------------------------------------------------------------------
//------------------------------------------------------------------------
...
export function _trackTimerDelta(beforeSecs, afterSecs) {
    const delta = afterSecs - beforeSecs;
    if (delta > 0) globalThis._levelTimeAdded += delta;
    else if (delta < 0) globalThis._levelTimeLost += -delta;
}
...
export function _applyLowTimeVignette() {
    ...
    if (!globalThis.SETTINGS.lowTimeVignette || globalThis.timerFrozen || window.LEVEL_FLAGS.goldenClockActive) return;
...
export function _applyLowHealthVignette() {
    ...
    const healthEnabled = (typeof globalThis.SETTINGS.lowHealthVignette !== 'undefined')
        ? globalThis.SETTINGS.lowHealthVignette
        : globalThis.SETTINGS.lowTimeVignette;
    if (!healthEnabled) return;

    const max = (typeof globalThis.playerMaxHP !== 'undefined' && globalThis.playerMaxHP > 0) ? globalThis.playerMaxHP : 100;
    const curHP = (typeof globalThis.playerCurrentHP !== 'undefined') ? globalThis.playerCurrentHP : max;
    if (max <= 0) return;
...
export function _egClearCenterGridBanners(exceptId) {
    var ids = [
...
function _showLowTimeWarningBanner(tier) {
    ...
    if (typeof globalThis.showToast === 'function') globalThis.showToast(toastText, toastColors[tier] || '#facc15');
...
export function _resetLowTimeWarningState() {
...
export function updTimer() {
    ...
    if (typeof globalThis.PassiveTracker !== 'undefined') globalThis.PassiveTracker.onTimerTick();
...
export function timesUp() {
    ...
    document.getElementById('lose-sub').textContent =
        `${globalThis.mistakeCount} ${globalThis.mistakeCount !== 1 ? t('ov_win_mistakes') : t('ov_win_mistake')}. ${t('btn_retry2')}!`;
...
function _getPoissonInterval() {
    if (!globalThis.ptHasSkill('poisson_process_1') &&
        !globalThis.ptHasSkill('poisson_process_2') &&
        !globalThis.ptHasSkill('poisson_process_3')) return null;

    if (globalThis.ptHasSkill('poisson_process_3')) return 60;
    if (globalThis.ptHasSkill('poisson_process_2')) return 90;
    return 120;
}
...
function _initSkillTimers() {
    ...
    _timedStasisNext = null;
    _lawOfLargeNext = null;
    window._poissonNext = null;

    // timed_stasis (195-197): first auto-freeze after 10 minutes.
    if (globalThis.ptHasSkill('timed_stasis_1')) {
        _timedStasisNext = Date.now() + 10 * 60 * 1000;
    }

    // keystone_law_of_large_numbers (219): first trigger after 5 minutes.
    if (globalThis.ptHasSkill('keystone_law_of_large_numbers')) {
        _lawOfLargeNext = Date.now() + 5 * 60 * 1000;
    }
...
function _initProceduralSystems() {
    globalThis._ergodicFieldInit();
    globalThis._randomWalkInit();
    if (typeof globalThis.resetMarkovianState === 'function') globalThis.resetMarkovianState();
}
...
function _tickPassiveSkills() {
    globalThis._poissonProcessTick();
    globalThis._ergodicFieldTick();
    globalThis._entropyDrainTick();
    globalThis._randomWalkTick();
    globalThis._degreesOfFreedomTick();
    if (typeof globalThis._markovSnapshotTick === 'function') globalThis._markovSnapshotTick();
}
...
function _calcTimedStasisDuration() {
    let dur = 1000;
    if (globalThis.ptHasSkill('timed_stasis_2')) dur += 500;
    if (globalThis.ptHasSkill('timed_stasis_3')) dur += 500;
    return dur;
}
...
function _tickTimedStasis() {
    if (!globalThis.ptHasSkill('timed_stasis_1')) return;
    if (!_timedStasisNext) return;
    if (Date.now() < _timedStasisNext) return;
...
function _triggerLawOfLargeNumbers() {
    ...
    if (typeof globalThis._applyCellEffect === 'function' && revealedIds.length > 0) {
        globalThis._applyCellEffect(revealedIds, 'reveal');
    }
...
function _tickLawOfLargeNumbers() {
    if (!_lawOfLargeNext) return;
    if (Date.now() < _lawOfLargeNext) return;
...
function _tickCountdown() {
    if (window.LEVEL_FLAGS.goldenClockActive) return; // clock is paused
...
function _tickInventoryUnlock() {
    const elapsed = Math.floor((Date.now() - globalThis.levelStartTime) / 1000);
    const thresh = globalThis.curMods.timetrial ? 60 : 180;
    if (elapsed >= thresh && elapsed - 1 < thresh) globalThis.buildInventoryPanel();
}
...
export function _calcEmergencyScanDuration() {
    let dur = 2000;
    if (globalThis.ptHasSkill('emergency_scan_2')) dur += 1000;
    if (globalThis.ptHasSkill('emergency_scan_3')) dur += 2000;
    return dur;
}
...
function _tickEmergencyScan() {
    if (!globalThis.ptHasSkill('emergency_scan_1')) return;
...
export function startTimer() {
    ...
    timerInterval = setInterval(() => {
        ...
        if (globalThis.dead || globalThis.timerFrozen) return;
        ...
        if (typeof globalThis.triggerLowTimeBanterIfNeeded === 'function') globalThis.triggerLowTimeBanterIfNeeded();
...
export function resumeTimer() {
    if (!globalThis.dead && !timerInterval) startTimer();
}
...
if (typeof window !== 'undefined' && !_healthVignettePoll) {
    _healthVignettePoll = setInterval(() => {
