...
try { Object.defineProperty(globalThis, 'hoverCol', { get() { return hoverCol; }, set(v) { hoverCol = v; }, configurable: true }); } catch (e) {}
// mouse-over.js
...
export function _isDragCellAllowed(row, col) {
    ...
    if (globalThis.dragAxis === null && (row !== dragStartRow || col !== dragStartCol)) {
        if (row === dragStartRow) globalThis.dragAxis = 'row';
        else if (col === dragStartCol) globalThis.dragAxis = 'col';
        else globalThis.dragAxis = 'row';   // diagonal: default to row
    }

    // No axis locked yet (cursor hasn't moved) - allow the cell.
    if (globalThis.dragAxis === null) return true;
    if (globalThis.dragAxis === 'row' && row === dragStartRow) return true;
    if (globalThis.dragAxis === 'col' && col === dragStartCol) return true;
    return false;
}
...
export function onHover(e, row, col) {
    ...
    if (painting && !globalThis.dead && _isDragCellAllowed(row, col)) {
        applyCell(row, col);
    }
}
...
export function onHoverOut(row, col) {
...
export function dragCounterApply(row, col, count) {
...
export function dragCounterClear() {
